...
const changeDisplay = (showEl, hideEl, activeLink, inactiveLink) => {
  ...
  if (inactiveLink) inactiveLink.classList.add("active");
}
...
const editPost = async () => {
  changeDisplay(null, error)
  if (areInputsValid([titleInput,contentInput])){
    const blogPost = {body: contentInput.value,title: titleInput.value,}
    const stringifiedBlogPost = JSON.stringify(blogPost)
...
const renderComments = async blogPostId => {
  ...
  comments.map(comment =>new BlogComment(
        comment.name,
        comment.body,
        comment.email,
        comment.postId,
  ...
  ).forEach(comment => {commentsList.innerHTML += `
...
// EVENT HANDLERS
const setCommentBtnEventHandlers = posts =>posts
...
const setDeleteBtnEventHandlers = posts =>posts
